...
<template>
 <article class="dialog-select-tree">
   <base-dialog
   :title="dialogTitle"
   :modal-show.sync="visibleModal"
   :class="className"
   @close="handleClose"
   @update:modalShow="handleUpdate">
   <div>
     <el-tree
      slot="content"
      v-loading="loading"
   ...
   <div class="dialog-footer">
      ...
      <el-button type="primary" @click="handleMakeSure" size="small">确 定</el-button>
    </div>
   </base-dialog>
 </article>
</template>
...
export default {
  ...
  methods: {
    ...
    checkChange (data, checked, indeterminate) {
      if (this.lastCheckKey) {
        if (checked) {
          if (this.lastCheckKey !== data[this.nodeKey]) {
            this.$refs['selectTree'].setChecked(this.lastCheckKey, false)
          }
        }
      }
      if (checked) {
        this.lastCheckKey = data[this.nodeKey]
      }
    }
  },
